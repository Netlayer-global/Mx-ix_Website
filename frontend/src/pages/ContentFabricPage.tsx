            ...
            <div className="lg:col-span-7">
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                {CACHED_PLATFORMS.map((p) => (
                  <div
                    key={p.name}
                    className="group relative flex flex-col rounded-xl border border-gray-200 bg-white p-4 transition-all duration-300 hover:border-gray-300 hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] hover:-translate-y-0.5"
                  >
                    <div className="flex h-11 w-11 items-center justify-center rounded-lg bg-gray-50 transition-colors duration-300 group-hover:bg-white group-hover:shadow-sm">
                      <BrandMark name={p.name} className="h-6 w-6" />
                    </div>
                    <div className="mt-3.5 text-[15px] font-black tracking-[-0.03em] text-ink">
                      {p.name}
                    </div>
                    <div className="mt-1 font-mono text-[9px] uppercase leading-relaxed tracking-label text-gray-400">
                      {p.note}
                    </div>
                  </div>
                ))}
