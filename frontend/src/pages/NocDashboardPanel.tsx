      ...
      <header className="bg-gray-800 border-b border-gray-700 px-6 py-4 sticky top-0 z-20">
        <div className="max-w-5xl mx-auto flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-xl font-bold">NOC Operations</h1><p className="text-gray-500 text-sm">Fabric health at a glance</p></div>
          </div>
        </div>
      </header>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
          {[
            ...
            <div key={s.label} className="bg-gray-800 border border-gray-700 rounded-lg p-4">
              <div className="text-[10px] uppercase tracking-wider text-gray-500 font-mono">{s.label}</div>
              ...
              {data.atRisk.map((m) => (
                <div key={m.id} className="flex items-center gap-3 bg-gray-900 border border-gray-700 rounded p-3">
                  <div className="flex-1 min-w-0">
                    <div className="font-bold text-sm truncate">{m.name} {m.asn ? <span className="text-gray-500 font-normal">AS{m.asn}</span> : ''}</div>
