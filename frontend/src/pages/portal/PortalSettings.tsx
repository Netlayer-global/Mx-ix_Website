...
const PortalSettings: React.FC<Props> = ({ user, org }) => {
  ...
  const Row: React.FC<{ label: string; value?: React.ReactNode }> = ({ label, value }) => (
    <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 last:border-0">
      <span className="font-mono text-label-sm tracking-label uppercase text-gray-500">{label}</span>
        ...
        {enabled ? (
          <div className="space-y-3">
            <p className="text-sm text-gray-500 flex items-center gap-2">
              <Smartphone className="w-4 h-4 text-gray-500" /> Your account is protected with an authenticator app.
            </p>
            <input
              type="password"
