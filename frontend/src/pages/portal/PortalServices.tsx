        ...
        <div className="p-5 space-y-4">
          {tab === 'new_port' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className="block font-mono text-label-sm tracking-label uppercase text-gray-500 mb-2">Location</label>
              ...
              <div>
                <label className="block font-mono text-label-sm tracking-label uppercase text-gray-500 mb-2">Port speed</label>
          ...
          {tab === 'upgrade' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className="block font-mono text-label-sm tracking-label uppercase text-gray-500 mb-2">Port to upgrade</label>
                {ports.length ? (
                  ...
                  <p className="text-sm text-gray-500 py-3">You have no ports to upgrade yet.</p>
                )}
              </div>
              <div>
                <label className="block font-mono text-label-sm tracking-label uppercase text-gray-500 mb-2">New speed</label>
                <select value={speed} onChange={(e) => setSpeed(e.target.value)} className={inputClass}>
                  {catalog?.speeds.map((s) => (
                    <option key={s} value={s}>
                      {s}
          ...
          {tab === 'addon' && (
            <div className="space-y-3">
              ...
              {ports.length > 0 && (
                <div>
                  <label className="block font-mono text-label-sm tracking-label uppercase text-gray-500 mb-2">Apply to port (optional)</label>
                  <select value={portId} onChange={(e) => setPortId(e.target.value)} className={inputClass}>
                  ...
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="font-bold text-sm truncate">
                        {typeLabel[o.type]} <span className="text-gray-500 font-normal">· {summary}</span>
                      </p>
                  ...
                  <div className="flex items-center gap-3">
                    {!['completed', 'rejected', 'cancelled'].includes(o.status) && (
                      <span
                        ...
                        className="font-mono text-label-sm tracking-mono uppercase text-gray-500 hover:text-[#F20732] transition-colors cursor-pointer flex items-center gap-1"
                      >
