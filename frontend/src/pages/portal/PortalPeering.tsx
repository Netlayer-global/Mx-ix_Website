        ...
        {([
          ...
          <button
            ...
            className={`px-4 py-3 font-mono text-label-sm tracking-mono uppercase transition-colors hover-trigger relative ${
              tab === t.id ? 'text-ink' : 'text-gray-500 hover:text-ink'
            }`}
          >
            {t.label}
...
const SessionsTab: React.FC<Props> = ({ org }) => {
  ...
  const [routePagination, setRoutePagination] = useState<any>(null);
  ...
  const loadRoutes = useCallback(async (s: PortalSession, f: RouteFilter, page = 0, q = '') => {
    setRoutesLoading(true);
    setRoutes([]);
    const res = await portalApi.getPeeringRoutes(s.routeserverId, s.neighborId, f, page, q);
    if (res.success && res.data) {
      const d = res.data as any;
      const list: RouteRow[] = d.imported || d.filtered || d.routes || d.not_exported || [];
      setRoutes(list);
      setRoutePagination(d.pagination || null);
      setRoutePage(page);
    }
  ...
  const toggle = (s: PortalSession) => {
    ...
    loadRoutes(s, 'received', 0, '');
  };

  const switchFilter = (s: PortalSession, f: RouteFilter) => {
    setFilter(f);
    loadRoutes(s, f, 0, routeQuery);
  };

  const searchRoutes = (s: PortalSession, q: string) => {
    setRouteQuery(q);
    loadRoutes(s, filter, 0, q);
  };

  const goPage = (s: PortalSession, p: number) => {
    loadRoutes(s, filter, p, routeQuery);
  };
                ...
                >
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="font-mono text-xs text-gray-500 mt-0.5 truncate">{s.address}</p>
                    </div>
                  </div>
                ...
                {isOpen && (
                  <div className="border-t border-gray-200">
                    <div className="px-5 py-3 flex items-center justify-between gap-3 bg-gray-50 flex-wrap">
                      ...
                      <form
                        onSubmit={(e) => { e.preventDefault(); searchRoutes(s, routeQueryInput.trim()); }}
                      ...
                      >
                        <input
                          ...
                          className="px-3 py-1.5 font-mono text-xs text-ink placeholder-gray-400 outline-none w-44 max-w-[55vw]"
                        />
                        <button type="submit" className="px-2.5 py-1.5 bg-ink text-white hover:bg-[#F20732] transition-colors" aria-label="Search">
                          <Search className="w-3.5 h-3.5" />
                        </button>
                        {routeQuery && (
                          <button type="button" onClick={() => { setRouteQueryInput(''); searchRoutes(s, ''); }} className="px-2.5 py-1.5 text-gray-400 hover:text-[#F20732] transition-colors" aria-label="Clear">
                    ...
                    {routesLoading ? (
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="border-b border-gray-200 text-left">
                              <th className="px-5 py-3 font-mono text-label-sm tracking-label uppercase text-gray-500 font-normal">Prefix</th>
                              <th className="px-5 py-3 font-mono text-label-sm tracking-label uppercase text-gray-500 font-normal">AS Path</th>
                              {filter === 'filtered' ? (
                                <th className="px-5 py-3 font-mono text-label-sm tracking-label uppercase text-gray-500 font-normal">Reason</th>
                              ) : (
                                <th className="px-5 py-3 font-mono text-label-sm tracking-label uppercase text-gray-500 font-normal">Communities</th>
                              )}
                            </tr>
                          </thead>
                        ...
                        <div className="px-5 py-3 flex items-center justify-between flex-wrap gap-3 border-t border-gray-100">
                          <span className="font-mono text-[11px] tracking-label uppercase text-gray-500">
                            {routePagination && routePagination.total_results != null
                              ? `Page ${routePage + 1} of ${Math.max(1, routePagination.total_pages || 1)} · ${Number(routePagination.total_results).toLocaleString()} prefixes`
                              : `${routes.length} prefixes`}
                          </span>
                          {routePagination && (routePagination.total_pages || 0) > 1 && (
                            <div className="flex items-center gap-2 font-mono text-xs">
                              <button
                                disabled={routePage === 0}
                                onClick={() => goPage(s, routePage - 1)}
                                className="px-3 py-1.5 border border-gray-200 disabled:opacity-40 hover:border-[#F20732] hover:text-[#F20732] transition-colors"
                              >
                                Prev
                              </button>
                              <span className="text-gray-500">Page {routePage + 1} / {Math.max(1, routePagination.total_pages || 1)}</span>
                              <button
                                disabled={routePage >= (routePagination.total_pages || 1) - 1}
                                onClick={() => goPage(s, routePage + 1)}
                                className="px-3 py-1.5 border border-gray-200 disabled:opacity-40 hover:border-[#F20732] hover:text-[#F20732] transition-colors"
                              >
                                Next
                      ...
                      <p className="px-5 py-10 text-center font-mono text-label-sm tracking-mono uppercase text-gray-500">
                        {routeQuery ? 'No prefixes match that filter' : `No ${filter.replace('-', ' ')} prefixes`}
                      </p>
                    )}
                  </div>
                )}
      ...
      {showNew && (
        <section className="bg-white border border-gray-200 p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setShowNew(false)} className="text-gray-500 hover:text-ink"><X className="w-4 h-4" /></button>
          </div>
          ...
          <div className="bg-white border border-gray-200 divide-y divide-gray-100">
            {incoming.map((r) => (
              <div key={r.id} className="px-5 py-4 flex items-center gap-4">
                <ArrowDownLeft className="w-4 h-4 text-gray-500 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-sm">AS{r.fromAsn} <span className="text-gray-500 font-normal">wants to peer</span></p>
          ...
          <div className="bg-white border border-gray-200 divide-y divide-gray-100">
            {outgoing.map((r) => (
              <div key={r.id} className="px-5 py-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-sm truncate">{r.toName} <span className="text-gray-500 font-normal">AS{r.toAsn}</span></p>
                  {r.responseMessage && <p className="text-xs text-gray-500 mt-0.5 truncate">Reply: "{r.responseMessage}"</p>}
                </div>
                <Badge tone={reqTone(r.status)}>{r.status}</Badge>
                {r.status === 'pending' && (
                  <button onClick={() => cancel(r.id)} className="font-mono text-label-sm uppercase tracking-mono text-gray-500 hover:text-[#F20732] transition-colors hover-trigger">Cancel</button>
                )}
              </div>
            ))}
          </div>
              ...
              <div key={n.id} className="group bg-white border border-gray-200 p-4 hover:border-gray-300 transition-colors">
                <div className="flex items-start justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="font-mono text-xs text-gray-500 mt-0.5">AS{n.asn} · {n.type}</p>
                  </div>
  ...
  return (
    <section className="bg-white border border-gray-200 p-6 max-w-2xl">
      ...
      <div className="space-y-4">
        <div>
          <label className="block font-mono text-label-sm tracking-label uppercase text-gray-500 mb-2">Policy</label>
        ...
        <div>
          <label className="block font-mono text-label-sm tracking-label uppercase text-gray-500 mb-2">Policy URL</label>
          <input
        ...
        <div>
          <label className="block font-mono text-label-sm tracking-label uppercase text-gray-500 mb-2">Notes</label>
          <textarea
