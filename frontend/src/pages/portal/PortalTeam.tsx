    ...
    <div>
      ...
      {!isAdmin && (
        <div className="border border-gray-200 bg-gray-50 px-5 py-3 mb-6 flex items-center gap-2 text-sm text-gray-500">
          <ShieldCheck className="w-4 h-4 text-gray-500" /> You have {user.role} access — only admins can change team
          membership.
        </div>
      )}
      ...
      {showAdd && isAdmin && (
        <section className="bg-white border border-gray-200 p-5 mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setShowAdd(false)} className="text-gray-500 hover:text-ink">
              <X className="w-4 h-4" />
            </button>
          </div>
              ...
              <div key={m.id} className="px-5 py-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-sm truncate">
                    {m.name} {self && <span className="text-gray-500 font-normal">(you)</span>}
                    {!m.isActive && <span className="ml-2 font-mono text-label-sm uppercase text-gray-500">disabled</span>}
                  </p>
                ...
                {isAdmin && !self && (
                  <>
                    <button
                      ...
                      className="font-mono text-label-sm tracking-mono uppercase text-gray-500 hover:text-ink transition-colors hover-trigger"
                    >
                      {m.isActive ? 'Disable' : 'Enable'}
                    </button>
                    <button onClick={() => remove(m)} className="p-1.5 text-gray-500 hover:text-[#F20732] transition-colors">
