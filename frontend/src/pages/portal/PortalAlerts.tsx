          ...
          {rules.map((r) => (
            <div key={r._id} className="bg-white border border-gray-200 p-4">
              <div className="flex items-center gap-4">
                ...
                <button onClick={() => test(r._id)} className="flex items-center gap-1 font-mono text-label-sm uppercase tracking-mono text-gray-500 hover:text-[#F20732] transition-colors hover-trigger">
                  <Zap className="w-3.5 h-3.5" /> Test
                </button>
                <button onClick={() => toggle(r)} className="font-mono text-label-sm uppercase tracking-mono text-gray-500 hover:text-ink transition-colors hover-trigger">
                  {r.enabled ? 'Disable' : 'Enable'}
                </button>
                <button onClick={() => remove(r._id)} className="p-1.5 text-gray-500 hover:text-[#F20732] transition-colors">
