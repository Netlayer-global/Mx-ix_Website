...
/** Eyebrow label in the premium editorial style. */
...
export const PageHeading: React.FC<{ title: string; subtitle?: string; eyebrow?: string }> = ({
  ...
  <div className="mb-8">
    ...
    <h2 className="text-[clamp(1.8rem,3.5vw,2.5rem)] font-black tracking-[-0.04em] leading-[0.92] text-ink">{title}</h2>
    {subtitle && <p className="mt-4 max-w-2xl text-sm leading-7 text-gray-600">{subtitle}</p>}
  </div>
);
...
export const StatCard: React.FC<{
  ...
  <div className="group relative bg-white border border-gray-200 p-6 overflow-hidden hover:border-gray-300 transition-colors">
    ...
    <span className="font-mono text-label-sm tracking-label uppercase text-gray-500 group-hover:text-ink transition-colors">
      {label}
    </span>
    <div className="text-4xl font-light tracking-tighter text-ink mt-3 flex items-baseline">
      <CountUp value={value} />
      {suffix && <span className="text-sm ml-2 text-gray-500 font-mono font-bold tracking-label">{suffix}</span>}
    </div>
