...
const MembersPage: React.FC<MembersPageProps> = ({ onNavigate }) => {
  ...
  const filtered = useMemo(() => {
    ...
    let list = members.filter((m) => {
      ...
      if (q && !m.name.toLowerCase().includes(q) && !String(m.asn || '').includes(q) && !(m.locations || []).some((loc) => loc.toLowerCase().includes(q))) return false;
      return true;
    });
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-12">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            ...
            <div className="relative max-w-xs w-full">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name, ASN or city" className="w-full border border-gray-200 pl-9 pr-3 py-2 font-mono text-xs focus:outline-none focus:border-[#F20732] transition-colors" />
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="py-16 text-center border border-gray-200 border-dashed font-mono text-xs text-gray-500 uppercase tracking-label">{error}</div>
          ) : !members.length ? (
            <div className="py-20 text-center border border-gray-200 border-dashed">
              <Users className="w-10 h-10 text-gray-300 mx-auto mb-4" />
              <p className="font-mono text-xs text-gray-500 uppercase tracking-label">Member directory coming soon.</p>
            </div>
          ) : (
                  ...
                  {filtered.map((m) => (
                    <tr key={m._id} className="border-t border-gray-100 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-right">
                        {m.website ? (
                          <a href={m.website} target="_blank" rel="noopener noreferrer" className="inline-flex text-gray-500 hover:text-[#F20732] transition-colors" title="Website"><ExternalLink className="w-4 h-4" /></a>
                        ) : <span className="text-gray-300">—</span>}
                      </td>
                    </tr>
                  ))}
                  {!filtered.length && (
                    <tr><td colSpan={8} className="px-5 py-16 text-center font-mono text-xs text-gray-500 uppercase tracking-label">No members match your filter.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          {!loading && members.length > 0 && (
            <div className="font-mono text-label-sm tracking-label uppercase text-gray-500 mt-3">
