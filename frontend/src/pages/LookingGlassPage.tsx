...
const LookingGlassPage: React.FC = () => {
  ...
  const [routePagination, setRoutePagination] = useState<LgPagination | null>(null);
  ...
  const fetchRoutes = useCallback(
    async (neighbor: LgNeighbor, filter: RouteFilter, rsId: string, page = 0, q = '') => {
      setRoutesLoading(true);
      setRoutesError('');
      const res = await lookingGlassApi.getRoutes(rsId, neighbor.id, filter, page, q);
      if (res.success && res.data) {
        setRoutes(pickRoutes(res.data));
        setRoutePagination(res.data.pagination || null);
        setRoutePage(page);
  ...
  const handleRouteTab = (tab: RouteFilter) => {
    setRouteTab(tab);
    if (selectedNeighbor) fetchRoutes(selectedNeighbor, tab, selectedRs, 0, routeQuery);
  };

  const searchRoutes = (q: string) => {
    setRouteQuery(q);
    if (selectedNeighbor) fetchRoutes(selectedNeighbor, routeTab, selectedRs, 0, q);
  };

  const goRoutePage = (p: number) => {
    if (selectedNeighbor) fetchRoutes(selectedNeighbor, routeTab, selectedRs, p, routeQuery);
  };
  ...
  const handleLookup = async (e: React.FormEvent) => {
    ...
    if (searchMode === 'asn') {
      ...
      if (res.success && res.data) {
        ...
        setRoutes(r);
        if (!r.length) {
          const init = (res.data as any)?.api?.store_status?.routes?.initialized;
          setRoutesError(
            init === false
              ? 'Global prefix search is still initializing on the route servers (the route store has not finished syncing yet). Meanwhile, open a route server from a location below and use the prefix filter there.'
              : 'No routes found for that prefix across the fabric.'
          );
        }
      } else {
      ...
      <section className="relative pt-36 md:pt-44 pb-12 bg-ink text-white overflow-hidden">
        ...
        <div className="relative max-w-[1400px] mx-auto px-6 md:px-12">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="eyebrow">Looking Glass</span>
          </div>
            ...
            <div className="flex border border-white/15 w-fit">
              {(['prefix', 'asn'] as SearchMode[]).map((m) => (
                <button key={m} type="button" onClick={() => setSearchMode(m)}
                  className={`px-4 py-1.5 font-mono text-label-sm font-bold tracking-label uppercase transition-colors ${searchMode === m ? 'bg-[#F20732] text-white' : 'text-gray-500 hover:text-white'}`}>
          ...
          {lookupMode ? (
            lookupKind === 'asn' ? (
              <div>
                ...
                {routesLoading ? (
                        ...
                        {asnResults.map(({ rs, neighbor }, i) => {
                          ...
                          return (
                            <tr key={`${rs.id}-${neighbor.id}-${i}`} onClick={() => openFromLookup(rs, neighbor)} className="border-t border-gray-100 cursor-pointer hover:bg-gray-50 transition-colors hover-trigger group">
                              ...
                              <td className="px-4 py-3 font-mono text-xs text-gray-700 whitespace-nowrap">{rs.name.replace(/\s*\((IPv4|IPv6)\)/i, '')} <span className="text-gray-500">{isV6(rs) ? 'v6' : 'v4'}</span></td>
                  ...
                  <div className="py-16 text-center border border-gray-200 border-dashed"><p className="font-mono text-xs text-gray-500 uppercase tracking-label">{routesError || 'No matching sessions.'}</p></div>
                )}
              </div>
            ) : (
              <div>
            ...
            <>
                ...
                <>
                      ...
                      {locations.map(([group, list]) => {
                        ...
                        return (
                          <button key={group} onClick={() => openLocation(group)} className="group relative text-left p-7 border border-gray-200 hover:border-[#F20732] hover:bg-gray-50 transition-colors hover-trigger overflow-hidden">
                            ...
                            <p className="font-mono text-label-sm tracking-label uppercase text-gray-500">{list.length} Route Servers · {v4}× v4 · {v6}× v6</p>
                          </button>
                        );
                      })}
                    </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-12">
                    ...
                    <div className="border border-gray-200 p-7">
                      ...
                      <p className="mt-5 pt-5 border-t border-gray-100 font-mono text-[10px] leading-relaxed text-gray-500">
              ...
              {view === 'neighbors' && (
                <>
                  ...
                  <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-3">
                    <div>
                      <div className="font-mono text-label-sm tracking-mono uppercase text-gray-500 mb-1">{selectedGroup}</div>
              ...
              {view === 'neighbor' && selectedNeighbor && (
                <NeighborDetail neighbor={selectedNeighbor} rsName={selectedRsObj?.name || ''} onBack={() => { setView('neighbors'); updateHash(selectedRs); }} routeTab={routeTab} onRouteTab={handleRouteTab} routes={routes} routesLoading={routesLoading} routesError={routesError} routeQuery={routeQuery} onSearch={searchRoutes} pagination={routePagination} routePage={routePage} onPage={goRoutePage} />
              )}
            </>
          )}
        </div>
...
const StatChip: React.FC<{ label: string; value: number; accent?: boolean }> = ({ label, value, accent }) => (
  <div className={`px-4 py-2 border ${accent ? 'border-[#F20732]/30 bg-[#F20732]/5' : 'border-gray-200'}`}>
    ...
    <div className="font-mono text-[9px] tracking-label uppercase text-gray-500">{label}</div>
  </div>
);
...
const CopyBtn: React.FC<{ text: string }> = ({ text }) => {
  ...
  return (
    <button onClick={(e) => { e.stopPropagation(); copyText(text).then((ok) => { if (ok) { setDone(true); setTimeout(() => setDone(false), 1200); } }); }} className="opacity-0 group-hover:opacity-100 transition-opacity text-gray-500 hover:text-[#F20732]" title="Copy">
...
const NeighborsTable: React.FC<{
  ...
  if (!neighbors.length) return <div className="py-16 text-center border border-gray-200 border-dashed"><p className="font-mono text-xs text-gray-500 uppercase tracking-label">No BGP sessions found.</p></div>;
  return (
    <div className="border border-gray-200 overflow-x-auto">
      <table className="w-full text-left min-w-[820px]">
        <thead className="bg-ink text-white">
...
const NeighborDetail: React.FC<{
  ...
  routeQuery: string; onSearch: (q: string) => void;
  pagination: LgPagination | null; routePage: number; onPage: (p: number) => void;
}> = ({ neighbor, rsName, onBack, routeTab, onRouteTab, routes, routesLoading, routesError, routeQuery, onSearch, pagination, routePage, onPage }) => {
  ...
  return (
    <>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-y divide-gray-100">
          {facts.map((f) => (
            <div key={f.label} className="p-5">
              <div className="font-mono text-[9px] tracking-label uppercase text-gray-500 mb-1.5">{f.label}</div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          >
            <input
              ...
              placeholder="Filter prefix (e.g. 103.0.0.0/24)"
              className="px-3 py-2 font-mono text-xs text-black placeholder-gray-400 outline-none w-56 max-w-[60vw]"
            />
            <button type="submit" className="px-3 py-2 bg-ink text-white hover:bg-[#F20732] transition-colors" aria-label="Search routes">
              <Search className="w-4 h-4" />
            </button>
            {routeQuery && (
              <button type="button" onClick={() => onSearch('')} className="px-3 py-2 text-gray-400 hover:text-[#F20732] transition-colors" aria-label="Clear filter">
                <X className="w-4 h-4" />
              </button>
            )}
          </form>
      ...
      <RoutesTable
        ...
        error={routesError}
        empty={routeQuery ? 'No routes match that prefix.' : routeTab === 'received' ? 'No routes received from this neighbor.' : 'No filtered routes.'}
        server={
          pagination && (pagination.total_pages || 0) > 0
            ? { page: routePage, totalPages: pagination.total_pages || 1, total: pagination.total_results || routes.length, onPage }
            : undefined
        }
      />
    </>
...
const RoutesTable: React.FC<{
  ...
  error: string;
  empty: string;
  server?: { page: number; totalPages: number; total: number; onPage: (p: number) => void };
}> = ({ routes, loading, error, empty, server }) => {
  ...
  if (!routes.length) return <div className="py-16 text-center border border-gray-200 border-dashed"><p className="font-mono text-xs text-gray-500 uppercase tracking-label">{empty}</p></div>;

  const usingServer = !!server;
  const pages = usingServer ? Math.max(1, server!.totalPages || 1) : Math.ceil(routes.length / ROUTES_PER_PAGE);
  const curPage = usingServer ? server!.page : page;
  const start = usingServer ? 0 : page * ROUTES_PER_PAGE;
  const slice = usingServer ? routes : routes.slice(start, start + ROUTES_PER_PAGE);
  ...
  return (
    <div>
              ...
              return (
                <React.Fragment key={`${r.network}-${idx}`}>
                  <tr onClick={() => setExpanded(open ? null : idx)} className="border-t border-gray-100 hover:bg-gray-50 transition-colors cursor-pointer group hover-trigger">
                    ...
                    <td className="px-4 py-3 font-mono text-[11px] text-gray-500">{formatCommunities(r.bgp?.communities)}</td>
                  </tr>
                  {open && (
                      ...
                      <td colSpan={4} className="px-4 py-4">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 font-mono text-[11px]">
                          ...
                          <div className="col-span-2 md:col-span-4">
                            <div className="text-gray-500 uppercase tracking-label mb-1">Large Communities</div>
      ...
      <div className="flex items-center justify-between mt-3 flex-wrap gap-3">
        <span className="font-mono text-label-sm tracking-label uppercase text-gray-500">
          {usingServer
            ? `Page ${curPage + 1} of ${pages}${server!.total ? ` · ${server!.total.toLocaleString()} routes` : ''}`
        ...
        {pages > 1 && (
          <div className="flex items-center gap-2 font-mono text-xs">
            <button
              disabled={curPage === 0}
              onClick={() => (usingServer ? server!.onPage(curPage - 1) : setPage((p) => Math.max(0, p - 1)))}
              className="px-3 py-1.5 border border-gray-200 disabled:opacity-40 hover:border-[#F20732] hover:text-[#F20732] transition-colors"
            >
              Prev
            </button>
            <span className="text-gray-500">Page {curPage + 1} / {pages}</span>
            <button
              disabled={curPage >= pages - 1}
              onClick={() => (usingServer ? server!.onPage(curPage + 1) : setPage((p) => Math.min(pages - 1, p + 1)))}
              className="px-3 py-1.5 border border-gray-200 disabled:opacity-40 hover:border-[#F20732] hover:text-[#F20732] transition-colors"
            >
              Next
            </button>
...
const Attr: React.FC<{ label: string; value: any }> = ({ label, value }) => (
  <div>
    <div className="text-gray-500 uppercase tracking-label mb-1">{label}</div>
