...
const LocationsPage: React.FC<LocationsPageProps> = ({ preSelectedLocation }) => {
  ...
  // ── Browser back/forward support for the city detail view ──
  const openCity = (loc: AnyLoc) => {
    setSelected(loc);
    try { window.history.pushState({ mxLocation: loc.id }, '', `/locations/${loc.id}`); } catch { /* noop */ }
  };
  const closeCity = () => {
    if (/^\/locations\/.+/.test(window.location.pathname)) window.history.back();
    else setSelected(null);
  };
  useEffect(() => {
    const onPop = () => {
      const m = window.location.pathname.match(/^\/locations\/(.+)$/);
      if (m) { const loc = locations.find((l) => l.id === decodeURIComponent(m[1])); setSelected(loc || null); }
      else setSelected(null);
    };
  ...
  if (selected) {
    return <LocationDetail loc={selected} onBack={closeCity} onRequest={goToContact} />;
  }
  ...
  return (
    <div className="min-h-screen bg-white text-ink">
      ...
      <section className="sticky top-0 z-20 bg-white/90 backdrop-blur-md border-b border-gray-200">
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 py-3 space-y-2">
          <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide">
            <span className="font-mono text-label-sm tracking-label uppercase text-gray-500 mr-2 flex-shrink-0 w-16">Region</span>
          ...
          {countriesInRegion.length > 1 && (
            <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide">
              <span className="font-mono text-label-sm tracking-label uppercase text-gray-500 mr-2 flex-shrink-0 w-16">Country</span>
              <button
      ...
      <section className="max-w-[1400px] mx-auto px-6 md:px-12 py-12 md:py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {visible.map((loc) => {
            ...
            return (
              <button
                key={loc.id}
                onClick={() => live && openCity(loc)}
                className={`group relative bg-white border border-gray-200 p-6 text-left overflow-hidden transition-colors ${live ? 'hover:bg-gray-50 hover-trigger cursor-pointer' : 'cursor-default'}`}
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className="font-mono text-label-sm tracking-label uppercase text-gray-500">{loc.region}</span>
                </div>
                <h3 className="text-2xl font-black tracking-tighter text-ink">{loc.name}</h3>
                <p className="font-mono text-xs text-gray-500 mt-1">{loc.country}{loc.ixName ? ` · ${loc.ixName}` : ''}</p>
                ...
                {live ? (
                  <div className="grid grid-cols-3 gap-3 mt-5 pt-5 border-t border-gray-100">
                    <div>
                      ...
                      <div className="font-mono text-label-sm tracking-label uppercase text-gray-500">Networks</div>
                    </div>
                    <div>
                      <div className="text-lg font-light tracking-tight text-ink tabular-nums">{loc.enabledSites?.length ?? loc.sites ?? 0}</div>
                      <div className="font-mono text-label-sm tracking-label uppercase text-gray-500">Data Centers</div>
                    </div>
                    <div>
                      <div className="text-lg font-light tracking-tight text-ink tabular-nums">{loc.capacity && loc.capacity !== '-' ? loc.capacity : '—'}</div>
                      <div className="font-mono text-label-sm tracking-label uppercase text-gray-500">Port Capacity</div>
                    </div>
                  </div>
                ) : (
                  <div className="mt-5 pt-5 border-t border-gray-100">
                    <span className="font-mono text-label-sm tracking-mono uppercase text-gray-500">{loc.established || 'Coming soon'}</span>
                  </div>
                )}
                {live && (
        ...
        {!visible.length && <p className="text-center py-16 font-mono text-label tracking-label uppercase text-gray-500">No locations in this region yet</p>}
      </section>

      {/* Why peer here */}
      <section className="border-t border-gray-200 bg-gray-50">
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 py-14">
          <span className="eyebrow">Why MX-IX</span>
      ...
      <section className="bg-ink text-white">
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 py-14 flex flex-col md:flex-row md:items-center md:justify-between gap-6 relative overflow-hidden">
          ...
          <div className="relative">
            ...
            <p className="text-gray-500 mt-2 max-w-xl">One port, one connection — reach every network on the fabric.</p>
          </div>
...
const LocationDetail: React.FC<{ loc: AnyLoc; onBack: () => void; onRequest: (city?: string) => void }> = ({ loc, onBack, onRequest }) => {
  ...
  const facts = [
    ...
    { icon: Globe, label: 'Capacity', value: loc.capacity && loc.capacity !== '-' ? loc.capacity : undefined },
  ...
  return (
    <div className="min-h-screen bg-white text-ink">
      ...
      <section className="relative bg-ink text-white overflow-hidden pt-32 md:pt-36 pb-12">
        <div className="absolute -top-24 -right-24 w-96 h-96 rounded-full bg-[#F20732]/15 blur-[120px]" />
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 relative z-10">
          <button onClick={onBack} className="inline-flex items-center gap-2 font-mono text-label-sm tracking-mono uppercase text-gray-500 hover:text-white transition-colors mb-8 hover-trigger">
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            {facts.length > 0 && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {facts.map((f) => (
                  <div key={f.label} className="bg-white border border-gray-200 p-4">
                    <f.icon className="w-4 h-4 text-[#F20732] mb-2" />
                    <div className="font-mono text-label-sm tracking-label uppercase text-gray-500">{f.label}</div>
            ...
            {loc.address && (
              <div className="flex items-start gap-3 border border-gray-200 p-5">
                ...
                <div>
                  <div className="font-mono text-label-sm tracking-label uppercase text-gray-500 mb-1">Facility Address</div>
            ...
            {pricing.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {pricing.map((p, i) => (
                    <div key={i} className="group relative bg-white border border-gray-200 p-5 overflow-hidden hover:bg-gray-50 transition-colors">
                      <div className="absolute top-0 left-0 w-full h-1 bg-[#F20732] -translate-x-full group-hover:translate-x-0 transition-transform duration-500" />
                      <div className="font-mono text-label tracking-label uppercase text-gray-500">{p.portSpeed} Port</div>
                      ...
                      <div className="mt-3 pt-3 border-t border-gray-100 flex items-center justify-between">
                        <span className="font-mono text-label-sm tracking-label uppercase text-gray-500">Setup</span>
                  ...
                  {routeServers.map((rs, i) => (
                    <div key={i} className="border border-gray-200 p-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="font-mono text-xs text-gray-500">AS{rs.asn}</span>
                      </div>
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between gap-2"><span className="font-mono text-label-sm tracking-label uppercase text-gray-500">IPv4</span><CopyChip value={rs.ipv4} /></div>
                        <div className="flex items-center justify-between gap-2"><span className="font-mono text-label-sm tracking-label uppercase text-gray-500">IPv6</span><CopyChip value={rs.ipv6} /></div>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  {sites.map((s, i) => (
                    <div key={i} className="border border-gray-200 p-4">
                      <div className="flex items-center justify-between">
                        ...
                        {s.status && <span className="font-mono text-label-sm tracking-mono uppercase text-gray-500">{s.status === 'available' ? 'Available' : 'Soon'}</span>}
                      </div>
                      <p className="font-mono text-xs text-gray-500 mt-0.5">{s.provider}</p>
            ...
            {asns.length > 0 && (
              <div>
                <div className="flex items-center justify-between gap-4 mb-4 flex-wrap">
                  <h3 className="font-mono text-label tracking-label uppercase text-ink flex items-center gap-2"><Network className="w-4 h-4 text-[#F20732]" /> Connected Networks <span className="text-gray-500">({asns.length})</span></h3>
                  {asns.length > 4 && (
                    <div className="relative">
                      <Search className="w-4 h-4 text-gray-500 absolute left-3 top-1/2 -translate-y-1/2" />
                      <input value={asnQuery} onChange={(e) => setAsnQuery(e.target.value)} placeholder="Search name or ASN…" className="pl-9 pr-3 py-2 border border-gray-300 font-mono text-sm focus:outline-none focus:border-[#F20732] w-56" />
                    </div>
                  )}
                </div>
                  ...
                  {filteredAsns.map((a, i) => (
                    <div key={i} className="px-4 py-3 flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <span className="font-mono text-xs text-gray-500 ml-2">AS{a.asnNumber}</span>
                        {a.macro && <span className="font-mono text-xs text-gray-500 ml-2">{a.macro}</span>}
                      </div>
                      <span className="font-mono text-label-sm tracking-mono uppercase text-gray-500 flex-shrink-0">{a.peeringPolicy}</span>
                    </div>
                  ))}
                  {!filteredAsns.length && <p className="px-4 py-6 text-center font-mono text-label-sm tracking-mono uppercase text-gray-500">No match</p>}
                </div>
              </div>
            )}
          </div>
          ...
          <aside className="lg:col-span-1">
            <div className="lg:sticky lg:top-24 space-y-6">
              ...
              <div className="bg-ink text-white p-6 relative overflow-hidden">
                ...
                <p className="text-gray-500 text-sm mb-4">Get availability and pricing for a port at this location.</p>
