      ...
      <header className="bg-gray-800 border-b border-gray-700 px-6 py-4 sticky top-0 z-20">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-xl font-bold">Email Templates</h1><p className="text-gray-500 text-sm">Manage transactional email content</p></div>
          </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
        {editing && (
          <section className="bg-gray-800 border border-gray-700 rounded-lg p-5 space-y-3">
            ...
            {editing.variables?.length ? <p className="text-xs text-gray-500 font-mono">Variables: {editing.variables.map((v) => `{{${v}}}`).join(' ')}</p> : null}
            <div className="flex justify-end gap-2">
              <button onClick={() => setEditing(null)} className="px-4 py-2 text-sm text-gray-500 hover:text-white">Cancel</button>
          ...
          {items.map((t) => (
            <div key={t._id} className="bg-gray-800 border border-gray-700 rounded-lg p-4 flex items-center gap-4">
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-gray-500 truncate">{t.subject || 'No subject'}</div>
              </div>
              <button onClick={() => setEditing(t)} className="text-xs font-mono uppercase text-gray-500 hover:text-white">Edit</button>
              <button onClick={() => remove(t._id)} className="p-2 text-gray-500 hover:text-[#F20732]"><Trash2 className="w-4 h-4" /></button>
            </div>
          ))}
