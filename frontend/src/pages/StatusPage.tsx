...
const fmtDuration = (startMs: number, endMs: number) => {
  let mins = Math.max(0, Math.floor((endMs - startMs) / 60000));
  const w = Math.floor(mins / 10080); mins %= 10080;
  ...
  if (m && !w) parts.push(`${m} minute${m > 1 ? 's' : ''}`);
...
const StatusPage: React.FC = () => {
  ...
  const openIncident = (inc: IncidentItem) => {
    ...
    try { window.history.pushState({ mxIncident: inc._id }, '', `/status/${inc._id}`); } catch { /* noop */ }
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  const closeIncident = () => {
    if (/^\/status\/.+/.test(window.location.pathname)) window.history.back();
    else setSelected(null);
  };

  if (selected) return <IncidentDetail inc={selected} onBack={closeIncident} />;
    ...
    <div className="min-h-screen bg-white">
          ...
          {loading ? (
            ...
            <>
                    ...
                    <div key={group} className="border border-gray-200">
                      <button onClick={() => setCollapsed((p) => ({ ...p, [group]: !p[group] }))} className="w-full flex items-center justify-between px-5 py-4 hover:bg-gray-50 transition-colors hover-trigger">
                        <span className="flex items-center gap-3">
                          {isCollapsed ? <Plus className="w-4 h-4 text-gray-500" /> : <Minus className="w-4 h-4 text-gray-500" />}
              ...
              {activeIncidents.length > 0 && (
                <div className="mt-12">
                  <div className="flex items-center justify-between mb-4 border-b border-gray-200 pb-2">
                    ...
                    <span className="font-mono text-[10px] text-gray-500">Times shown in your local timezone</span>
                  </div>
                  <div className="space-y-4">{activeIncidents.map((inc) => <IncidentCard key={inc._id} inc={inc} onOpen={() => openIncident(inc)} />)}</div>
                </div>
              )}
              ...
              {pastIncidents.length > 0 && (
                <div className="mt-12">
                  <h2 className="font-mono text-label-sm font-bold tracking-label uppercase text-gray-500 mb-4 border-b border-gray-200 pb-2">Incident History</h2>
                  <div className="space-y-4">{pastIncidents.map((inc) => <IncidentCard key={inc._id} inc={inc} resolved onOpen={() => openIncident(inc)} />)}</div>
                </div>
              )}
            </>
          )}
      ...
      {popup && (
        <>
          ...
          <div
            className={`fixed z-[100] w-64 bg-white border border-gray-200 shadow-elevated p-4 ${pinned ? 'pointer-events-auto' : 'pointer-events-none'}`}
          ...
          >
            ...
            <div className="font-mono text-[10px] uppercase tracking-label text-gray-500 mb-2">{popup.component}</div>
              ...
              <div className="space-y-2.5">
                {popup.incidents.map((inc) => (
                  <button
                    ...
                    className="block w-full text-left group/inc hover-trigger"
                  >
                    <div className={`text-xs font-bold group-hover/inc:underline ${IMPACT_TEXT[inc.impact]?.split(' ')[0] || 'text-gray-700'}`}>
                ...
                {pinned && <div className="font-mono text-[9px] text-gray-400 uppercase tracking-label pt-1">Click an event for details</div>}
              </div>
            ) : (
...
const IncidentCard: React.FC<{ inc: IncidentItem; resolved?: boolean; onOpen?: () => void }> = ({ inc, resolved, onOpen }) => (
  <div className={`border-l-4 border border-gray-200 p-5 ${resolved ? 'border-l-green-500' : 'border-l-[#F20732] bg-[#F20732]/[0.02]'}`}>
    <div className="flex items-start gap-3 mb-2 flex-wrap">
      ...
      <button onClick={onOpen} className="font-bold text-black flex-1 text-left hover:text-[#F20732] transition-colors hover-trigger">{inc.title}</button>
    </div>
    <div className="space-y-2 mt-3 mb-3">
      {inc.updates.map((u, i) => (
        <div key={i} className="flex gap-3 text-sm">
          <span className="font-mono text-[10px] text-gray-500 whitespace-nowrap pt-0.5 w-28 flex-shrink-0">{fmtDate(u.timestamp)}</span>
          <div>
    ...
    <div className="flex items-center justify-between gap-2 pt-3 border-t border-gray-100 flex-wrap">
      ...
      <button onClick={onOpen} className="font-mono text-[10px] font-bold uppercase tracking-label text-[#F20732] hover:underline hover-trigger flex items-center gap-1">
        View details →
      </button>
    </div>
  </div>
...
// Full incident detail page (opened from a day popup or an incident card).
const IncidentDetail: React.FC<{ inc: IncidentItem; onBack: () => void }> = ({ inc, onBack }) => {
  useEffect(() => { window.scrollTo(0, 0); }, []);
  const startMs = new Date(inc.startedAt || inc.createdAt).getTime();
  const endMs = inc.resolvedAt ? new Date(inc.resolvedAt).getTime() : Date.now();
  const startStr = new Date(startMs).toLocaleString(undefined, { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' });
  const tz = Intl.DateTimeFormat().resolvedOptions().timeZone;
  return (
    <div className="min-h-screen bg-white text-ink">
      {/* Hero */}
      <section className="relative pt-36 md:pt-44 pb-10 bg-ink text-white overflow-hidden">
        <div className="absolute -top-24 -right-24 w-96 h-96 rounded-full bg-[#F20732]/15 blur-[120px]"></div>
        <div className="relative max-w-[900px] mx-auto px-6 md:px-12">
          <button onClick={onBack} className="inline-flex items-center gap-2 font-mono text-label-sm tracking-mono uppercase text-gray-400 hover:text-white transition-colors mb-6 hover-trigger">
            <ArrowLeft className="w-4 h-4" /> Back to status
          </button>
          <h1 className="text-3xl md:text-5xl font-black tracking-tighter leading-[0.95] mb-5">{inc.title}</h1>
          <div className="flex flex-wrap items-center gap-2">
            <span className={`font-mono text-label-sm font-bold uppercase tracking-mono px-3 py-1.5 border bg-white ${IMPACT_TEXT[inc.impact] || 'text-gray-600 border-gray-300'}`}>{inc.impact} incident</span>
            {inc.affectedComponents.map((c) => (
              <span key={c} className="font-mono text-label-sm tracking-mono uppercase px-3 py-1.5 border border-white/25 text-gray-200">{c}</span>
            ))}
            <span className="inline-flex items-center gap-2 font-mono text-label-sm tracking-mono uppercase px-3 py-1.5 border border-white/25 text-gray-200">
              <Clock className="w-3.5 h-3.5" /> {startStr} · {fmtDuration(startMs, endMs)}
            </span>
          </div>
        </div>
      </section>
      ...
      {/* Updates */}
      <section className="bg-white py-12 min-h-[40vh]">
        <div className="max-w-[900px] mx-auto px-6 md:px-12">
          <div className="flex items-center justify-between mb-6 border-b border-gray-200 pb-2">
            <h2 className="font-mono text-label tracking-label uppercase text-ink">Updates</h2>
            <span className="font-mono text-[10px] text-gray-500">Times shown in {tz} timezone</span>
          </div>
          ...
          {inc.updates.length ? (
              ...
              {inc.updates.map((u, i) => (
                <div key={i} className="p-6">
                  <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
                    <h3 className={`text-lg font-bold capitalize ${INCIDENT_STATUS_TEXT[u.status] || 'text-ink'}`}>{u.status}</h3>
                    <span className="font-mono text-[11px] text-gray-500">{new Date(u.timestamp).toLocaleString(undefined, { month: 'long', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                  </div>
                  <div className="space-y-2.5">
                    {String(u.message).split('\n').filter((p) => p.trim()).map((p, j) => (
            ...
            <p className="font-mono text-xs text-gray-500 uppercase tracking-label">No updates posted for this incident.</p>
          )}

          <button onClick={onBack} className="mt-8 inline-flex items-center gap-2 font-mono text-label-sm font-bold tracking-mono uppercase text-ink hover:text-[#F20732] transition-colors hover-trigger">
            <ArrowLeft className="w-4 h-4" /> Back to status
          </button>
        </div>
      </section>
