      ...
      <main className="max-w-5xl mx-auto px-6 py-8 space-y-8">
        ...
        <section className="bg-gray-800 border border-gray-700 rounded-lg p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            {editingId && <button onClick={resetForm} className="flex items-center gap-1 text-sm text-gray-500 hover:text-white"><X className="w-4 h-4" /> Cancel</button>}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-wider text-gray-500 font-mono mb-1">Member since</label>
          ...
          {members.map((m) => (
            <div key={m._id} className="bg-gray-800 border border-gray-700 rounded-lg p-4 flex items-center gap-4">
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-gray-500">{m.asn ? `AS${m.asn} · ` : ''}{m.type} · {m.peeringPolicy}{m.locations?.length ? ` · ${m.locations.join(', ')}` : ''}</div>
              </div>
              <button onClick={() => startEdit(m)} className="p-2 text-gray-500 hover:text-white transition-colors"><Edit2 className="w-4 h-4" /></button>
              <button onClick={() => remove(m._id)} className="p-2 text-gray-500 hover:text-[#F20732] transition-colors"><Trash2 className="w-4 h-4" /></button>
            </div>
          ))}
