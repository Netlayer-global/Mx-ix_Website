...
const OnboardingPage: React.FC<OnboardingPageProps> = ({ onNavigate }) => {
  ...
  const sites = selectedLoc?.enabledSites || [];

  // Reset data center when the location changes
  useEffect(() => { setDatacenter(''); }, [location]);

  const allChecked = CHECKS.every((c) => checks[c.key]);

  const canNext = () => {
    if (step === 1) return companyName.trim().length > 1;
    if (step === 2) return !!location && !!speed && (sites.length === 0 || !!datacenter);
  ...
  const submit = async () => {
    setSubmitting(true);
    setError('');
    const dcName = sites.find((s: any) => s.id === datacenter)?.name;
    const notes = `Onboarding: ${type} · Port ${speed} @ ${selectedLoc?.name || location}${dcName ? ` (${dcName})` : ''} · Cross-connect ordered by customer.`;
        ...
        <section className="max-w-[1400px] mx-auto px-6 md:px-12 py-14">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              ...
              <div key={s.n} className="bg-white border border-gray-200 p-8">
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <React.Fragment key={s.id}>
                <div className={`flex items-center gap-2 flex-shrink-0 ${active ? 'text-ink' : complete ? 'text-[#F20732]' : 'text-gray-500'}`}>
        ...
        <div className="border border-gray-200 p-6 md:p-10">
          ...
          {step === 2 && (
            <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {liveLocations.map((l) => (
                      <button
                        ...
                        className={`text-left p-4 border border-gray-200 transition-colors hover-trigger ${location === l.id ? 'bg-ink text-white' : 'bg-white hover:bg-gray-50'}`}
                      >
                        <div className="font-bold">{l.name}</div>
                        <div className={`font-mono text-xs mt-0.5 ${location === l.id ? 'text-gray-300' : 'text-gray-500'}`}>{l.country}{l.ixName ? ` · ${l.ixName}` : ''}</div>
                      </button>
                    ))}
                  </div>
                )}
                ...
                <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                  {speedOptions.map((s: string) => (
                    <button
                      ...
                      type="button"
                      onClick={() => setSpeed(s)}
                      className={`py-4 font-mono text-base font-bold tracking-tighter border border-gray-200 transition-colors hover-trigger ${speed === s ? 'bg-[#F20732] text-white' : 'bg-white text-ink hover:bg-gray-50'}`}
                    >
                      {s}
                    </button>
                  ))}
              ...
              {/* Data center */}
              <div>
                <label className={labelCls}>Data center {sites.length > 0 ? '*' : ''}</label>
                {!location ? (
                  <div className="border border-dashed border-gray-300 p-4 font-mono text-sm text-gray-500">Select a location first</div>
                ) : sites.length === 0 ? (
                  <div className="border border-dashed border-gray-300 p-4 text-sm text-gray-500">No data centers listed for this location yet — our team will advise on hand-off options.</div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {sites.map((s: any) => (
                      <button
                        key={s.id}
                        type="button"
                        onClick={() => setDatacenter(s.id)}
                        className={`text-left p-4 border border-gray-200 transition-colors hover-trigger ${datacenter === s.id ? 'bg-ink text-white' : 'bg-white hover:bg-gray-50'}`}
                      >
                        <div className="font-bold text-sm">{s.name}</div>
                        {s.provider && <div className={`font-mono text-xs mt-0.5 ${datacenter === s.id ? 'text-gray-300' : 'text-gray-500'}`}>{s.provider}</div>}
                      </button>
                    ))}
                  </div>
                )}
              ...
              <div className="bg-white border border-gray-200 border-l-2 border-l-[#F20732] p-4">
                <p className="text-sm text-gray-600 leading-relaxed">
                  <span className="font-bold text-ink">Cross-connect:</span> you arrange and order the cross-connect to the MX-IX fabric directly with your chosen data center. We'll confirm the exact hand-off details (rack, port, LOA) once your application is reviewed.
                </p>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-6">
              ...
              <p className="text-xs text-gray-500">Not sure about something? You can still proceed and our NOC will help you get set up.</p>
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              <div className="grid sm:grid-cols-2 gap-2">
                {[
                  ...
                  ['Data center', sites.find((s: any) => s.id === datacenter)?.name || '—'],
                  ['Port speed', speed],
                  ['Cross-connect', 'Ordered by customer'],
                  ['Contact', contactName],
                  ['Email', email],
                  ['Phone', phone || '—'],
                ].map(([k, v]) => (
                  <div key={k} className="bg-white border border-gray-200 p-4">
                    <div className="font-mono text-label-sm tracking-label uppercase text-gray-500">{k}</div>
          ...
          <div className="flex items-center justify-between mt-10 pt-6 border-t border-gray-200">
            ...
            {step < 5 ? (
              <button
                onClick={next}
                disabled={!canNext()}
                className={`px-7 py-3 font-mono text-label-sm font-bold tracking-mono uppercase transition-colors flex items-center gap-2 hover-trigger ${canNext() ? 'bg-ink text-white hover:bg-[#F20732]' : 'bg-gray-200 text-gray-500 cursor-not-allowed'}`}
              >
