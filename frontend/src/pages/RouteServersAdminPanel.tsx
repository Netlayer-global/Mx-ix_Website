      ...
      <header className="bg-gray-800 border-b border-gray-700 px-6 py-4 sticky top-0 z-20">
        <div className="max-w-5xl mx-auto flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-xl font-bold">Route Servers</h1><p className="text-gray-500 text-sm">Alice-LG sources &amp; config</p></div>
          </div>
          ...
          {servers.map((s) => (
            <div key={s._id} className="bg-gray-800 border border-gray-700 rounded-lg p-4 flex items-center gap-4">
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-gray-500">{s.group || '—'} · {s.backend} · {s.apiUrl}</div>
              </div>
              <button onClick={() => toggle(s)} className="text-xs font-mono uppercase text-gray-500 hover:text-white">{s.enabled ? 'Disable' : 'Enable'}</button>
              <button onClick={() => remove(s._id)} className="p-2 text-gray-500 hover:text-[#F20732]"><Trash2 className="w-4 h-4" /></button>
            </div>
          ))}
