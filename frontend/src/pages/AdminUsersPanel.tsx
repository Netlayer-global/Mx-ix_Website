      ...
      <header className="bg-gray-800 border-b border-gray-700 px-6 py-4 sticky top-0 z-20">
        <div className="max-w-4xl mx-auto flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-xl font-bold">Admin Users</h1><p className="text-gray-500 text-sm">Role-based admin access</p></div>
          </div>
        </div>
      </header>
        ...
        <section className="space-y-2">
          {users.map((u) => (
            <div key={u.id} className="bg-gray-800 border border-gray-700 rounded-lg p-4 flex items-center gap-4">
              ...
              <button onClick={() => toggle(u)} className="text-xs font-mono uppercase text-gray-500 hover:text-white">{u.isActive ? 'Disable' : 'Enable'}</button>
              <button onClick={() => remove(u.id)} className="p-2 text-gray-500 hover:text-[#F20732]"><Trash2 className="w-4 h-4" /></button>
            </div>
          ))}
        </section>
      </main>
