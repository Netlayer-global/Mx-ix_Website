...
// Fallback values used only until the backend / admin-configured stats load.
// Kept at a realistic exchange scale (Gbps) so the UI never shows inflated
// figures such as "195 Tbps".
...
export const generateMockTrafficData = (): TrafficDataPoint[] => {
  ...
  for (let i = 23; i >= 0; i--) {
    ...
    const baseValue = 780; // Gbps
    const variation = Math.random() * 240 - 120; // ±120 Gbps variation

    data.push({
      timestamp: timestamp.toISOString(),
      value: Math.max(420, baseValue + variation)
    });
  }
...
export const fetchNetworkStats = async (): Promise<NetworkStat[]> => {
  try {
    ...
    if (response.success && response.data) {
      ...
      // Traffic metrics arrive in the unit reported by the collector (Gbps), so the
      // unit is carried over with the value — otherwise a 985 Gbps peak would be
      // rendered against a stale "Tbps" label.
      const trafficUnit = metrics.traffic.unit || 'Gbps';
      return networkStats.map(stat => {
        switch (stat.id) {
          case 'total_traffic':
            return { ...stat, value: metrics.traffic.current, unit: trafficUnit };
          case 'peak_traffic':
            return { ...stat, value: metrics.traffic.peak, unit: trafficUnit };
          case 'active_as':
            return { ...stat, value: metrics.connections.active };
          case 'avg_latency':
            return { ...stat, value: metrics.latency.global, unit: metrics.latency.unit || stat.unit };
          case 'uptime':
  ...
  const cityStats: NetworkStat[] = [
    {
      id: 'total_capacity',
      label: 'Total Capacity',
      value: Math.floor(3200 * multipliers.traffic),
      unit: 'Gbps',
      format: 'number',
      category: 'network'
    },
    {
      id: 'peak_traffic',
      label: 'Peak Traffic (24h)',
      value: parseFloat((985 * multipliers.traffic).toFixed(1)),
      unit: 'Gbps',
      trend: 'up',
      trendValue: '+8.1%',
      format: 'decimal',
    ...
    },
    {
      id: 'total_peers',
      label: 'Connected Networks',
      value: Math.max(1, Math.floor(84 * multipliers.peers)),
      unit: 'Peers',
      trend: 'up',
      trendValue: '+6',
      format: 'number',
    ...
    },
    {
      id: 'ipv4_prefixes',
      label: 'IPv4 Prefixes',
      value: Math.floor(62400 * multipliers.peers),
      unit: 'Routes',
      trend: 'stable',
      format: 'number',
      category: 'network'
...
const generateCityTrafficData = (multiplier: number): TrafficDataPoint[] => {
  ...
  for (let i = 23; i >= 0; i--) {
    const timestamp = new Date(now.getTime() - i * 60 * 60 * 1000);
    const baseValue = 780 * multiplier; // Gbps
    const variation = Math.random() * 240 - 120; // ±120 Gbps variation

    data.push({
      timestamp: timestamp.toISOString(),
      value: Math.max(420 * multiplier, baseValue + variation)
    });
  }
