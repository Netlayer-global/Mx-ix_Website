...
export interface IPort extends Document {
  ...
  macAddress?: string;
  status: PortStatus;
  // Metric source linkage (Grafana/Zabbix per-port graphs)
  zabbixHostId?: string;   // Zabbix host name as it appears in the Grafana datasource
  zabbixInterface?: string; // optional interface name to scope the item filter (shared switches)
