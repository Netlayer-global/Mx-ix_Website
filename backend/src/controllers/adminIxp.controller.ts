...
export const ixpSync = async (req: Request, res: Response): Promise<void> => {
  ...
  // When true (default), members with no matching org are created as pending
  // customers so they flow into the panel for review.
  ...
  for (const m of members) {
    ...
    // Match by existing IXP link, primary ASN, or any additional ASN.
    let org = await Organization.findOne({ ixpManagerId: id });
    if (!org && asn) org = await Organization.findOne({ $or: [{ asn }, { additionalAsns: asn }] });
    ...
    if (org) {
      ...
      await Organization.create({
        ...
        type: 'ISP',
        status: 'pending',
        notes: 'Auto-created from IXP Manager sync',
      });
      created++;
    } else {
  ...
  await logAudit({
    ...
    action: 'ixp.sync',
    resource: 'IXPManager',
    after: { fetched: members.length, linked, created, unmatched: unmatched.length },
  });

  res.json({ success: true, data: { fetched: members.length, linked, created, unmatched } });
};
...
export const ixpImportPorts = async (req: Request, res: Response): Promise<void> => {
  ...
  for (const vi of mine) {
    ...
    // Auto-map the Zabbix host to the IXP switch name (admin can correct it in
    // the per-port Metrics editor if the Zabbix host name differs).
    if (vi.switchName) data.zabbixHostId = vi.switchName;
    if (existing) {
      // Don't overwrite a host the admin may have hand-tuned.
      if (!existing.zabbixHostId) delete data.zabbixHostId;
