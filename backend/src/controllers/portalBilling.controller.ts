...
export const listInvoices = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await zohoBooks.listInvoices(org.zohoContactId, org.zohoProfileKey);
    if (!result.ok) {
...
export const invoicePdf = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await zohoBooks.getInvoicePdf(org.zohoContactId, String(req.params.id), org.zohoProfileKey);
